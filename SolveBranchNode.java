...
public class SolveBranchNode
{
    ...
    public static PairAdjsWithStateProba[] array_pawsps;// 内容对上面同样，遍历的话比列表快
    public static int length_array_pawsps = 0;

    public static List<int[]> orginAndNew = new ArrayList<>();/// 存放邻接
...
//    public static int maxChromoNum = 0;
    ...
    public static void construct(TreeNode the_currentNode, String the_outPath, int the_nodeNum)
    {
        ...
        outPath = the_outPath;
        nodeNum = the_nodeNum;
        System.out.println("construct执行了");
    }
    ...
    public static void solve1()
    {
        ...
        getCurrentAdjs();//获取当前结点邻接
        getPawsps();// 获得邻接对状态
        getEndPawsps();// 获得最终的邻接对的概率
        getPathFromPawsps();  // 获取路径
        sb.append("Finished" + "\n");
    ...
    public static void getPawsps()//PairAdjsWithStateProba
    {
        String nn = "0,0", ny = "0,1", yn = "1,0", yy = "1,1", yyd = "1,1d";
        AdjOrTelomere adj1, adj2;
        int m=0, n=0, count=0;
        System.out.println("getPawsps");
        List<String> stateList = new ArrayList<>();// 存放着某邻接在各个叶子的状态
        int count0;
        int count1;
        int count2;
        int count3;
        ...
        double p_11;
        double p_11d;
        PairAdjsWithStateProba proba;// 伴随转移概率的邻接对类
        int size_adjsOfCurrentNode = currentNode.adjsOfNode.size();// 当前结点邻接的数量
        for(int i=0; i<size_adjsOfCurrentNode-1; i++)
        {
            for(int j=i+1; j<size_adjsOfCurrentNode; j++)
            {
                adj1 = currentNode.adjsOfNode.get(i);//获取了一个邻接
...
//                    continue;
    ...
    public static void getEndPawsps()
    {
        ...
        double frequencyOfState = 0, numerator = 0, denominator = 0;// 分子 numerator // 分母 denominator
        // get_P_state_Dx 可重用的变量
        ...
        for(int i=0; i<size_adjsOfCurrentNode; i++)
        {
            ...
            current.add(p11);
            current.add(p11d);
            classifiedPawsps.add(current);// 存放着结点的数据，每个数据分为状态值为11和11d的邻接对
        }
    ...
    public static double getLS(TreeNode currentNode, PairAdjsWithStateProba pawsp, String state, double[] frequency,
                               ...
                               String nn, String ny, String yn, String yy, String yyd)// 获取Lx(Sx) :该结点中Sx的状态概率
    {
        System.out.println(currentNode.name+",getLs 执行了");
...
//        Constant.getP();
    ...
    {
        System.out.println("get_P_state_Dx 执行了");
        frequencyOfState = 0;
        numerator = 0;
        denominator = 0;
        if((nn).equals(state))
    ...
    public static void getPathFromPawsps()
    {
        ...
        List<AdjOrTelomere> starts = new ArrayList<>();// 所有的可能的路径的开端，端粒结点
        ...
        for(AdjOrTelomere currentStart : starts)
        {
            ...
            mostGenesPaths = getTheMostGensPaths(paths, genes, maxGeneNum, indexsOfMaxGeneNumPath, count_forGetTheMostGensPaths, geneNumOfPath, size_indexsOfMaxGeneNumPath);//获得基因数最多
